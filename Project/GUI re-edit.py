...
def click_B1():
     ...
     register=Button(pop1,text="Register",font=("Copperplate gothic",10),fg="black",bg="Orange",command=click_register)
...
def click_B2():
     ...
     sign=Button(pop1,text="Register",font=("Copperplate gothic",10),fg="black",bg="light green",command=lambda:click_sign(B2))
     sign.place(x=175,y=90)
...
def click_register():
     ...
     if len(username)>0 and len(password)>0:
          ...
          if username in list_of_users:
               lalert=Label(pop1,text="Username already exists",font=("Copperplate gothic",10),bg="dark red",fg="white")
...
def click_sign(b):
     ...
     if b==B1:
          if len(username)>0 and len(password)>0:
               ...
               if username not in data:
                   lalert=Label(pop1,text="Username does not exist, Please sign up",font=("Copperplate gothic",10),bg="dark red",fg="white")
                   lalert.place(x=80,y=145)
     ...
     if b==B2:
          if len(username)>0 and len(password)>0:
                    ...
                    data=cur.fetchall()
                    if username not in data:
                       lalert=Label(pop1,text="Account does not exist",font=("Copperplate gothic",10),bg="dark red",fg="white")
                       lalert.place(x=80,y=145)
